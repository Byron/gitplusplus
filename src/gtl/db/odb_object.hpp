...
/** \ingroup ODBObject
  ...
  * The input object can work in a variety of modes. The stream is input from the outside, which is
  * when it is owned by someone else. If it is not provided, the only valid usage of the instance is 
  * if it is used with the serialize method which effectively generates temporary stream that is 
...
struct odb_input_object : public odb_basic_object<ObjectTraits>
{
	...
	//! \fn odb_input_object(typename traits_type::object_type, typename traits_type::size_type, stream_type* stream = 0, const key_type* key_pointer = 0)
	...
	//! \param stream if not 0, pointer to own stream. If 0, and a stream is requested, a temporary one
	...
	//! \throw odb_object_error if we have no attached stream. In that case, the instance
	//! was not initialzed with a key pointer, and serialize() was not called, which would
	//! have created a temporary stream
	stream_type* stream_pointer();
